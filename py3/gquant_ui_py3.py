...
from gquant_main_calibr_py3 import *
import os
import re
...
def output_set():
    output = tf.asksaveasfilename(initialdir = os.getcwd(),title = "Select Output Directory",filetypes = (("Comma Separated file","*.csv"),("all files","*.*")))
...
l3_1.place(x = 2, y = 10)
ms_mode = StringVar()
d_c1 = tkinter.ttk.Combobox(g2, width = 16, textvariable = ms_mode, state = 'readonly')
...
unit2 = StringVar()
d_c2 = tkinter.ttk.Combobox(g2, width = 5, textvariable = unit2, state = 'readonly')
...
l4_1.place(x = 2, y = 50)
unit3 = StringVar()
d_c3 = tkinter.ttk.Combobox(g2, width = 16, textvariable = unit3, state = 'readonly',)
...
delimi = StringVar()
d_l1 = tkinter.ttk.Combobox(g2, width = 16, textvariable = delimi, state = 'readonly',)
...
l_c = tkinter.ttk.Combobox(g3, width = 16, textvariable = label_mass_unit, state = 'readonly')
...
dv.place(x = 100, y = 62)
delta_mass_value = StringVar()
d_c = tkinter.ttk.Combobox(g3, width = 4, textvariable = delta_mass_value, state = 'readonly')
d_c['value'] = ('Da')
d_c.current(0)
...
gdb = StringVar()
gdb_cbb = tkinter.ttk.Combobox(g3, width = 16, textvariable = gdb, state = 'readonly')
...
def open_res_path():
    ...
    if res and os.path.exists(res):
        if os.path.isfile(res):
            res = os.path.dirname(os.path.realpath(res))
