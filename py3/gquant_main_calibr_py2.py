...
def glycan_db_with_mods(initial_gdb, derivatization = {}, mode = 'positive', adducts = {'H':1.0078,'Na':22.9898},is_full_de = False):
    ...
    l = len(initial_gdb)
    new_gdb = pd.DataFrame(columns = [u'Hex', u'HexNAc', u'NeuAc', u'Xyl', u'dHex', u'composition',
       u'mw_residue', u'mw_free_end', u'element_compos_free_end',
       u'derivative', u'adduct', u'mw', u'first', u'second', u'third',
       u'fourth', u'fifth', u'sixth', u'1st', u'2nd', u'3rd', u'4th', u'5th',
       u'6th'])
            ...
            if is_full_de:
                ...
                for i in xrange(l):
                    for adduct in adducts:
                        ...
                        origin_row['derivative'] = derivatization.keys()[0]
                        mw = origin_row['mw_free_end'] + derivatization.values()[0] + adducts[adduct]
                        origin_row['mw'] = mw
                ...
                if is_full_de:
                    for i in xrange(l):
                        for adduct in adducts:
                            ...
                            origin_row['derivative'] = derivatization.keys()[0]
                            mw = origin_row['mw_free_end'] + derivatization.values()[0] + adducts[adduct] - 2*PROTON
                            origin_row['mw'] = mw
...
#    mz_list, intens_list = zip(*sorted(spect.items()))
    # currently a list
    mz_list, intens_list = zip(*sorted(spect))
...
#    charge = 1.0
    ...
    for peak in peaks:
        ...
        if not delta_mass:
            ...
            for i in xrange(len(gdb_with_adduct)):
...
def gquant(match_results, full_envelop_list, gdb_with_adduct, spect_peak, delta_mass, quant_mode = 'highest' ):
    ...
    match_results.sort(columns = ['m/z', 'mz_diff_ppm', 'derivatization'])
...
def cal_noise_level(spect_lst, mode = 'median'):
    ...
    '''    
    if mode == 'mean':
        return 0.2*np.mean([i[1] for i in spect_lst])
    if mode == 'median':
        return 0.2*np.median([i[1] for i in spect_lst])
